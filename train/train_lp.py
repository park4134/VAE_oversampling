import torch
import torch.nn as nn
import torch.optim as optim
import torch.nn.functional as F
from torch.utils.data import DataLoader
from torch.utils.tensorboard import SummaryWriter
from tqdm.auto import tqdm

import os
import numpy as np
import argparse
import yaml

import sys
sys.path.append(os.path.dirname(os.path.dirname(__file__)))
from models.latent_policy import *
from utils import *

class LP_Trainer():
    def __init__(self):
        self.args = self.parse_args
        self.device = 'cuda:0'
        self.best_val_loss = float('inf')
        self.patience_counter = 0

    def parse_args(self):
        parser = argparse.ArgumentParser(description="Train a reinforcement learning agent")
        parser.add_argument("--version", type=str, help="Version of model", default='model')
        parser.add_argument("--env_name", type=str, help="Environment name", default='MountainCar-v0')
        parser.add_argument("--data_name", type=str, help="Expert's data name")
        parser.add_argument("--n_state", type=int, help="n_state", default=2)
        parser.add_argument("--n_latent_action", type=int, help="n_latent_action", default=3)
        parser.add_argument("--units", type=list, help="units", default=[64, 32])
        parser.add_argument("--units_p", type=list, help="units_p", default=[])
        parser.add_argument("--alpha", type=float, help="alpha of LeakyReLU", default=0.2)
        parser.add_argument("--batch", type=int, help="batch size", default=32)
        parser.add_argument("--epochs", type=int, help="epochs", default=1000)
        parser.add_argument("--lr", type=float, help="learning rate of optimizer", default=0.0002)
        parser.add_argument("--patience", type=int, help="patience for early stop", default=30)
        parser.add_argument("--val_ratio", type=float, help="Ratio of validation data", default=0.1)
        args = parser.parse_args()
        return args

    def init_data(self):
        data_path = os.path.join(os.getcwd(), 'data', 'state_pairs', self.args.env_name, self.args.data_name)
        data = np.load(data_path)

        val_idx = np.random.choice(len(data), len(data)*self.args.val_ratio, replace=False)
        train_idx = np.setdiff1d(np.arange(data), val_idx)

        train_data, val_data = data[train_idx], data[val_idx]

        del data

        train_dataset = Data(train_data)
        self.train_dataloader = DataLoader(train_dataset, batch_size=self.args.batch, shuffle=True, num_workers=8)
        val_dataset = Data(val_data)
        self.val_dataloader = DataLoader(val_dataset, batch_size=self.args.batch, shuffle=True, num_workers=8)
    
    def init_model(self):
        self.model = LatentPolicy(
            n_state=self.args.n_state,
            n_latent_action=self.args.n_latent_action,
            units=self.args.units,
            units_p=self.args.units_p,
            alpha=self.args.alpha
        )

        self.model.to(device=self.device)
        initialize_weights(self.model)
    
    def init_optimizer(self):
        # Initialize optimizers
        params_min = list(self.model.state_encoder.parameters()) + list(self.model.generator.parameters())
        self.optimizer_min = optim.Adam(params_min, lr=self.args.lr, betas=(0.5, 0.999))

        params_exp = list(self.model.state_encoder.parameters()) + list(self.model.policy.parameters())
        self.optimizer_exp = optim.Adam(params_exp, lr=self.args.lr, betas=(0.5, 0.999))

    def init_logger(self):
        # Initialize TensorBoard logger
        log_dir = os.path.join(os.getcwd(), 'logs', self.env_name, 'LP', self.args.version)
        if not os.path.isdir(log_dir):
            os.makedirs(log_dir)
        self.writer = SummaryWriter(log_dir=log_dir)

    def train_one_epoch(self, epoch):
        self.model.train()
        running_loss = 0.0
        train_progress = tqdm(self.train_dataloader, desc=f"Epoch [{epoch+1}/{self.args.epochs}]")

        for i, batch in enumerate(train_progress):
            s = batch['state'].to(device=self.device)
            delta_s = batch['delta_s'].to(device=self.device)

            self.optimizer_min.zero_grad()
            self.optimizer_exp.zero_grad()

            z_p, delta_s_hat = self.model(s)

            loss_min_value = loss_min(delta_s, delta_s_hat)
            loss_exp_value = loss_exp(delta_s, z_p, delta_s_hat)
            loss = loss_min_value + loss_exp_value

            loss.backward()
            self.optimizer_min.step()
            self.optimizer_exp.step()

            running_loss += loss.item()
            train_progress.set_postfix(Train_Loss=running_loss/(i+1))

            # Log losses to TensorBoard
            self.writer.add_scalar('Train/Loss_Min', loss_min_value.item(), epoch * len(self.train_dataloader) + i)
            self.writer.add_scalar('Train/Loss_Exp', loss_exp_value.item(), epoch * len(self.train_dataloader) + i)

        return running_loss / len(train_progress)

    def validate(self):
        self.model.eval()
        val_loss = 0.0

        with torch.no_grad():
            for batch in self.valid_dataloader:
                s = batch['state'].to(device=self.device)
                delta_s = batch['delta_s'].to(device=self.device)

                z_p, delta_s_hat = self.model(s)
                entropy = -torch.sum(F.softmax(z_p, dim=-1) * torch.log(F.softmax(z_p, dim=-1) + 1e-8), dim=1)
                loss = loss_min(delta_s, delta_s_hat) + loss_exp(delta_s, z_p, delta_s_hat) + self.lambda_epsilon * torch.mean(entropy)
                val_loss += loss.item()

        val_loss /= len(self.valid_dataloader)
        return val_loss

    def _save_model(self):
        self.save_path = os.path.join(os.getcwd(), 'runs', self.args.env_name, 'latent_policy', self.args.version)
        if not os.path.isdir(self.save_path):
            os.makedirs(self.save_path)
        torch.save(self.model.state_dict(), self.save_path)
        print('Model saved!')

    def _save_config(self):
        config = vars(self.args)
        with open(os.path.join(self.save_path, 'config.yaml'), 'w') as file:
            yaml.dump(config, file, default_flow_style=False)
    
    def train(self):
        for epoch in range(self.args.epochs):
            train_loss = self._train_one_epoch(epoch)
            val_loss = self._validate()

            # Update progress bar with validation loss
            print(f"Epoch [{epoch+1}/{self.args.epochs}] - Train Loss: {train_loss:.3f}, Val Loss: {val_loss:.3f}")

            # Early stopping and model saving
            if val_loss < self.best_val_loss:
                self.best_val_loss = val_loss
                self.patience_counter = 0
                self._save_model()
            else:
                self.patience_counter += 1
                if self.patience_counter >= self.args.patience:
                    self._save_config()
                    print('Early stopping!')
                    break
            self._save_config()

        print('Finished Training')

if __name__ == "__main__":
    trainer = LP_Trainer()
    trainer.train()